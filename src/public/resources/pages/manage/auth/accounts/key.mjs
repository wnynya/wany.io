...
new (class extends LapisScript {
  async load() {
    input.code = document.querySelector('#input-manage-auth-account-key-code');
    input.label = document.querySelector(
      '#input-manage-auth-account-key-label'
    );
    input.perm = document.querySelector('#input-manage-auth-account-key-perm');
    button.showCode = document.querySelector(
      '#button-manage-auth-account-key-code-show'
    );
    button.updateLabel = document.querySelector(
      '#button-manage-auth-account-key-label-update'
    );
    button.updatePermPut = document.querySelector(
      '#button-manage-auth-account-key-perm-update-put'
    );
    button.updatePermDelete = document.querySelector(
      '#button-manage-auth-account-key-perm-update-delete'
    );
    button.delete = document.querySelector(
      '#button-manage-auth-account-key-delete'
    );
    addEventListener();
    select();
  }
...
function select() {
  JSONGetRequest(
    `${global.api}/auth/accounts/${global.auid}/keys/${global.kid}`
  )
    .then((res) => {
      ...
      document.querySelector('#manage-auth-account-key .perms').innerHTML = '';
      if (data.permissions.length > 0) {
        for (const perm of data.permissions) {
          document
            .querySelector('#manage-auth-account-key .perms')
...
function updatePermPut() {
  ...
  JSONPutRequest(
    `${global.api}/auth/accounts/${global.auid}/keys/${global.kid}/permissions`,
    data
  )
    .then(() => {
