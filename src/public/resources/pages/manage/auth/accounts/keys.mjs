...
new (class extends LapisScript {
  async load() {
    button.insert = document.querySelector(
      '#button-manage-auth-account-keys-insert'
    );
    addEventListener();
    select();
  }
...
function insert() {
  ...
  JSONPostRequest(`${global.api}/auth/accounts/${global.auid}/keys`, {})
    .then((res) => {
      select();
...
function select() {
  JSONGetRequest(`${global.api}/auth/accounts/${global.auid}/keys`)
    .then((res) => {
      const data = res.body.data;

      document.querySelector('#manage-auth-account-keys .keys').innerHTML = '';
      if (data.length > 0) {
        for (let key of data) {
          document
            .querySelector('#manage-auth-account-keys .keys')
