...
function select() {
  JSONGetRequest(`${global.api}/auth/accounts/@me/sessions`).then((res) => {
    const data = res.body.data;

    document.querySelector('#you-sessions .sessions').innerHTML = '';
    for (let session of data) {
      if (session.current) {
        document
          .querySelector('#you-sessions .sessions')
    ...
    function sessionElement(session) {
      ...
      e.querySelector('button')
        ? e.querySelector('button').addEventListener('click', (event) => {
            ...
            JSONDeleteRequest(`${global.api}/auth/accounts/@me/sessions/${sid}`)
              .then((res) => {
